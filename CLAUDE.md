...
- Weaknesses (`weaknesses` table) are free text per employee, shown to the user as **Udviklingsområder** (development areas) and phrased as something to practise, never as a fault. The API assigns a random encouraging growth emoji from `schema.WEAKNESS_EMOJIS` on creation (keep them kind, since the guide may be shown to the employee, and stick to emoji older Windows fonts render). They are entered in the UI only (no CSV column) and shown on the profile and in the Samtaleguide.
