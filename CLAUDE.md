...
- Weaknesses (`weaknesses` table) are free text per employee; the API assigns a random funny emoji from `schema.WEAKNESS_EMOJIS` on creation. They are entered in the UI only (no CSV column) and shown on the profile and in the Samtaleguide.
